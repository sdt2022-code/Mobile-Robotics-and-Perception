...
for i=2:numodom
    ...
    x_dot_odom(i-1) = v_odom(i-1)*cos(theta_odom(i-1));
    x_odom(i) = x_odom(i-1) + x_dot_odom(i-1)*(t_odom(i) - t_odom(i-1));
    

    y_dot_odom(i-1) = v_odom(i-1)*sin(theta_odom(i-1));
    y_odom(i) = y_odom(i-1) + y_dot_odom(i-1)*(t_odom(i) - t_odom(i-1));
...
theta_odom_noise_free = theta_odom;
% set up plot
figure();
clf;
hold on;
...
for n=1:100
    ...
    theta_odom = wrapToPi(theta_odom);
    %plot(t_odom , theta_odom , 'r');   %% Uncomment this line and line (200 - 204)
    % to plot the noise on theta.
...
%hold on;
%plot(t_odom,theta_odom_noise_free,'b');
%axis([150 300 -4 4])
%xlabel('t [s]');
%ylabel('theta [rad]');
%title('heading');
...
for n=1:2
    ...
    for i=1:size(t_laser,1)
        ...
        x_vehicle = y_laser(i,:).*cos_angles;
        y_vehicle = y_laser(i,:).*sin_angles;

        % Transformation matrix from vehicle frame to inertial frame
        ...
        % position of vehicle 
        [pose_i] = T_iv *[x_vehicle; y_vehicle ; ones(1 , size(x_vehicle , 2))];
